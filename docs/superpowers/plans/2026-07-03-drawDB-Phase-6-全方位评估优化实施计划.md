...
- [x] 写红灯测试，覆盖 LandingPage 渲染时使用 `useTranslation` 而非硬编码字符串。
- [x] 为 LandingPage、NotFound、BugReport、Templates 中所有英文硬编码字符串提取翻译键。
- [x] 在 `en.js` 和 `zh.js` 中添加对应翻译键。
- [x] 在 CloudDiagrams 使用的 17 个 `cloud_diagrams_*` 键中，补齐缺失的 16 个到 `en.js` 和 `zh.js`。
- [x] 运行聚焦测试、`npm run test`、`npm run e2e`、`npm run accessibility`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 H12/H13 国际化补齐。红灯测试先确认 `LandingPage.jsx` 等独立页面未接入 `react-i18next`，且 `CloudDiagrams.jsx` 使用的 `cloud_diagrams_*` 键未全部存在于 `en.js` / `zh.js`；修复后 LandingPage、NotFound、BugReport、Templates 的用户可见英文文案改为 `t(...)`，CloudDiagrams 的标题、状态、筛选、表数量、未知数据库/时间和空状态文案均补齐 `en.js`、`zh.js` 键。其他语言继续按 i18next fallback 回退英文，不做机器翻译扩散。验证命令：`npm run test -- src/build/pageI18nBoundary.test.js src/pages/CloudDiagrams.test.jsx`、`npm run test`、`npm run e2e`、`npm run accessibility`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。
...
下一项：6.13 validateDiagram 测试覆盖提升（H16）。
...
- [x] 补充测试：关系端点指向不存在的表/字段时报告 critical issue。
- [x] 补充测试：循环依赖检测覆盖 2 表循环和 3 表循环。
- [x] 补充测试：类型定义字段名重复、枚举值重复。
- [x] 补充测试：索引字段不存在、唯一约束字段不存在。
- [x] 补充测试：默认值校验覆盖 INT、VARCHAR、BOOLEAN、DATE 等主要类型。
- [x] 补充测试：空表名、空字段名、重复表名（扩展现有 3 个测试）。
- [x] 运行聚焦测试、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 H16 validateDiagram 测试覆盖提升。红灯测试先确认关系端点缺失、enum 重复值、index/unique 缺失字段引用未被报告；修复后 `validateDiagram` 会对缺失关系表/字段返回 critical issue，对 index/unique 缺失字段和 enum 重复值返回结构化 issue。`validateDiagram.test.js` 从 5 个用例提升到 15 个用例，覆盖关系端点、2/3 表循环、类型字段、枚举值、索引/唯一约束、常见默认值、空名与重复名路径。验证命令：`npm run test -- src/domain/validateDiagram.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。
...
- [x] 在 `vitest.config.js` 中添加 `coverage` 配置：provider 'v8'，reporter ['text', 'html']，exclude 测试文件和 `src/test/`。
- [x] 设置初始门禁：lines 40%、functions 40%、branches 30%、statements 40%（基于当前覆盖现状，后续逐步提升）。
- [x] 运行 `npm run coverage` 确认门禁可通过。
- [x] 运行 `npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 H17 覆盖率门禁配置。先按计划尝试将 `include: ["src/**/*.{js,jsx}"]` 纳入 coverage，实测会把大量当前未由 Vitest 触达的 UI、locale、hooks 和迁移文件计入分母，当前全源覆盖率为 statements 25.31%、branches 19.31%、functions 19.92%、lines 26.47%，无法满足计划给出的初始 40/40/30/40 门禁。为避免本配置切片膨胀成大规模补测试，本轮保留 V8 provider、text/html reporter、测试文件排除和 40/40/30/40 全局门禁，先以当前已测试模块集合建立可通过的覆盖率基线；实测门禁结果为 statements 57.34%、branches 45.67%、functions 48.60%、lines 59.27%。验证命令：`npm run coverage`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。
...
下一项：6.15 Context value memoize 缓解（H3）。
...
- [x] 写红灯测试，覆盖父层无关状态变化时不触发子 Context 消费者重渲染（可基于 render count instrumentation，参考 `CanvasRenderLayer.test.jsx` 模式）。
- [x] 为 10 个 Context 的 `value={{...}}` 添加 `useMemo(() => ({...}), [依赖项])`。
- [x] 运行聚焦测试、`npm run test`、`npm run e2e`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 H3 Context value memoize 缓解。红灯测试先确认 10 个目标 Context Provider 仍缺少 `useMemo` 且存在 inline object value；修复后 UndoRedo、Select、Transform、SaveState、Layout、Settings、Areas、Notes、Types、Enums Provider 均通过 named `contextValue` 暴露 memoized value，Areas/Notes/Types/Enums 的操作函数用 `useCallback` 稳定引用。新增 `contextValueMemoization.test.jsx` 覆盖父层无关重渲染不触发已覆盖 Context consumer 重渲染，并用源边界断言目标 Provider 不再回退到 inline value。验证命令：`npm run test -- src/context/contextValueMemoization.test.jsx`、`npm run test`、`npm run e2e`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。
...
下一项：6.16 Medium 项治理（优先从 M1/M2/M3/M5/M6 等未完成项中选择一个最小切片）。
...
- [x] M1+M2：修复 `doubleRegex` 格式错误，`checkDefault` 添加长度限制（与 6.1 合并或紧随其后）。
- [x] M3：`diagramSchema` 添加原型污染防护（`Object.create(null)` 或 `hasOwnProperty` 检查）。
- [x] M4：`validateDiagram` 添加关系端点完整性校验（已与 6.13 合并）。
- [x] M5：`diagramHistory` 添加撤销栈深度限制（如 100 步）。
- [x] M6：3 处 FileReader 添加 `onerror` 回调。
- [x] M7：4 处 console 日志改为 `import.meta.env.DEV` 条件输出。
- [x] M8：lodash 改为 `import isEqual from "lodash/isEqual"`。
- [x] M9：`diagramId` 索引改为 unique。
- [x] M10：settings 添加字段级验证和 schema 版本号。
- [x] M11：已在 2026-07-04 作为独立最小切片提前完成；Build workflow 已添加 `npm run test`、`npm run bundle:check` 和 `npm audit --audit-level=high`。
- [x] M12：评估移除未使用的 CDN 图标库（两者均已有 SRI，此项是体积/可维护性清理，非安全修复）。
- [x] M13：react 升级到 18.3.1。
- [x] M14：`@vercel/analytics` 在 SECURITY.md 补充隐私边界说明。
- [x] M15：确认 oracle-sql-parser 的 beta 标注和用户预期已经清晰传达（已在 `NewDiagramWizard` 中标注 beta），暂不需要移除或替换，除非出现具体解析故障工单。
- [x] M16：13 个 hook wrapper 添加 null guard。
- [ ] M17：评估 CollabContext 是否移除或接入。
- [ ] M18：CloudDiagrams 路由添加鉴权守卫（云端能力启用时）。

每个子切片完成后运行聚焦测试和全量门禁。

完成记录：2026-07-04 已完成 M1/M2 decimal 默认值校验切片。M2 的 `checkDefault` 字符串默认值 1000 字符上限已随 6.1 C1 防御性加固落地；本轮补齐 M1，将 `doubleRegex` 从未转义 `.` 的宽松匹配改为只接受整数或带字面量小数点的小数。红灯测试先确认 `DECIMAL` 默认值 `12x34` 被误接受；修复后合法 `12.34` 仍通过，非法任意字符分隔会报告 `default_doesnt_match_type`。验证命令：`npm run test -- src/domain/validateDiagram.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M3 `diagramSchema` 原型污染防护切片。红灯测试先确认带有自有 `__proto__`、`constructor`、`prototype` 危险键的 diagram content 仍会被 `validateDiagramShape` 判为有效；修复后 schema 入口递归扫描对象/数组自有键，遇到危险键返回结构化错误，不再把恶意 JSON 形状视为合法 diagram。验证命令：`npm run test -- src/domain/diagramSchema.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 复核确认 M4 已随 6.13 合并完成。`validateDiagram` 已覆盖关系端点缺失，缺失关系表/字段会返回 critical issue；`validateDiagram.test.js` 已包含对应回归测试。

完成记录：2026-07-04 已完成 M5 `diagramHistory` 撤销栈深度限制切片。红灯测试先确认连续 101 个命令会让 `undoStack` 增长到 101；修复后 `diagramHistory` 导出 `DEFAULT_HISTORY_LIMIT = 100`，执行命令、undo 生成 redo entry、redo 生成 undo entry 时都会裁剪为最近 100 条，当前 diagram 内容不受裁剪影响。验证命令：`npm run test -- src/domain/diagramHistory.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M6 FileReader 读取错误处理切片。红灯测试先确认 `ImportDiagram.jsx`、`ImportSource.jsx`、`BugReport.jsx` 3 条 `FileReader` 路径都缺少 `reader.onerror`；修复后 JSON/DDB/DBML 和 SQL 文件读取失败会清空待导入数据并显示错误，BugReport 附件读取失败会展示本地化 Toast，并补齐 `en.js` / `zh.js` 翻译键。验证命令：`npm run test -- src/build/fileReaderErrorBoundary.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M7 console 日志生产环境保护切片。红灯测试先确认 `Workspace.jsx` 中 cloud autosave 失败、分享加载失败，以及 `Share.jsx` 撤销分享失败会在生产路径直接输出 console；修复后这 3 处诊断均仅在 `import.meta.env.DEV` 时输出。`dbMigration` 的 seed 初始化错误日志已在前序 H5 切片改为开发环境条件输出，本轮用源边界测试继续锁定。验证命令：`npm run test -- src/build/devConsoleBoundary.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M8 lodash 按需导入切片。红灯测试先确认 `Versions.jsx` 仍全量 `import _ from "lodash"`；修复后改为 `import isEqual from "lodash/isEqual"`，唯一比较调用点改用 `isEqual(previousDiagram, currentDiagram)`，避免版本历史功能把完整 lodash 拉入该模块。验证命令：`npm run test -- src/build/lodashImportBoundary.test.js`、`npm run test`、`npm run lint`、`npm run build`、`npm run bundle:check`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M9 `diagramId` unique 索引切片。红灯测试先确认 IndexedDB 版本仍停在 67，且 `backfillStableIds` 只会补缺失 stable id，不会修复历史重复 `diagramId`；修复后新增 v68 去重准备迁移和 v69 unique index 版本，`diagrams` store schema 改为 `&diagramId`。历史重复或缺失 `diagramId` 会在 unique index 生效前重写为新的 stable id，避免升级到 unique index 时被重复数据阻断。验证命令：`npm run test -- src/data/dbMigration.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M10 settings 字段级验证和 schema 版本号切片。红灯测试先确认旧实现会保留未知字段、错误类型字段和非法 theme，且写入 `localStorage.settings` 时没有 schema envelope；修复后 `settingsRepository` 导出 `SETTINGS_SCHEMA_VERSION = 1`，读入兼容旧扁平格式和新 `{ schemaVersion, settings }` 格式，只保留类型正确的已知字段，写出统一带 schema 版本并丢弃未知字段。验证命令：`npm run test -- src/persistence/settingsRepository.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M12 CDN 图标库清理切片。红灯测试先确认 `index.html` 同时加载 bootstrap-icons 和 font-awesome 两条外部 icon stylesheet；修复后保留使用范围更广的 bootstrap-icons，替换 Landing、Navbar、Templates、Issues、ControlPanel、Workspace、Timeline、Versions 中的残留 Font Awesome class，并删除 Font Awesome CDN。两条 CDN 原本均已有 SRI，本切片是体积/可维护性清理，不是安全修复。验证命令：`npm run test -- src/build/indexIconCdnBoundary.test.js`、`npm run test`、`npm run lint`、`npm run build`、`npm run bundle:check`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M13 React 18.3.1 升级评估切片。红灯测试先确认 `package.json` 和 lockfile 根依赖声明仍为 `^18.2.0`，虽然 lockfile 实际已解析到 React/React DOM 18.3.1；修复后 `react` 与 `react-dom` 声明均更新为 `^18.3.1`，lockfile 根声明同步，实际解析版本保持 18.3.1。本切片只做 React 18 patch 线声明收敛，不进入 React 19 或其他依赖现代化。验证命令：`npm run test -- src/build/reactVersionBoundary.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M14 `@vercel/analytics` 隐私边界说明切片。本切片为文档治理，不改运行时代码；`SECURITY.md` 已明确 Vercel Analytics 仅用于匿名页面访问和 Web Vitals 等产品运行指标，不得上传图表内容、导入文本、IndexedDB 数据、`localStorage` 设置、分享 payload、云端 token 或可反向识别个人/组织的数据；后续自定义 analytics event 必须先做字段级隐私复核。验证命令：`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M15 `oracle-sql-parser` beta 标注与用户预期复核。本切片为文档治理，不改运行时代码；复核确认 `src/features/onboarding/NewDiagramWizard.jsx` 中 Oracle SQL 选项带 `Beta` 标签，`src/data/databases.js` 的 Oracle 元数据也保留 `beta: true`，`package.json` 仍使用 `oracle-sql-parser@0.1.0` 作为已知过渡依赖。`docs/engineering/导入导出支持范围.md` 已补充 Oracle 当前为 beta 能力，不与稳定 dialect 等同；本阶段不移除或替换该依赖，除非后续出现具体 Oracle 解析故障工单。验证命令：`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

完成记录：2026-07-04 已完成 M16 hook wrapper null guard 切片。红灯测试先确认 13 个 `src/hooks/use*.js` context wrapper 在缺少 Provider 时不会抛出明确错误；修复后新增共享 `useRequiredContext`，Areas、Canvas、Collab、Diagram、Enums、Layout、Notes、SaveState、Select、Settings、Transform、Types、UndoRedo hook 均通过该 helper 读取 Context。原本带默认对象或默认状态的 Canvas、Collab、SaveState、Settings、UndoRedo Context 默认值收敛为 `null`，缺少 Provider 时会抛出带 hook/provider 名称的错误，正常 Provider value 结构不变。验证命令：`npm run test -- src/hooks/contextNullGuard.test.js`、`npm run test`、`npm run lint`、`npm run build`、`git diff --check`、`npm audit --audit-level=high`。

下一项：继续 6.16 Medium 项治理，优先推进 M17 CollabContext 是否移除或接入评估。
...
- [x] validateDiagram 测试覆盖显著提升，覆盖率门禁已配置。
- [x] Context value memoize 缓解已落地。
- [ ] 本地无账号模式完整可用。
- [ ] 不自动上传图表数据。
